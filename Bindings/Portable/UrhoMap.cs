...
namespace Urho
{
	...
	internal static class UrhoMap
	{
		...
		internal static CollisionShape2D get_CollisionShape2D(IntPtr handle, int stringHash)
		{
			return Runtime.LookupObject<CollisionShape2D>(urho_map_get_ptr(handle, stringHash));
		}

		internal static ParticleEffect2D get_ParticleEffect2D(IntPtr handle, int stringHash)
		{
			return Runtime.LookupObject<ParticleEffect2D>(urho_map_get_ptr(handle, stringHash));
		}

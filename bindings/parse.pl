...
while (<>){
	...
	while (<>){
		...
		print CS "        int hash$pc;\n";
		print CS "        int Hash$pc { get { if (hash$pc == 0){ hash$pc = urho_hash_get_$pc (); } return hash$pc; }};\n";
		print CS "        public $cspt $pn => urho_get_$cspt (handle, );\n";
	    }
	    if (/}/){
		print CS "    }\n\n";
